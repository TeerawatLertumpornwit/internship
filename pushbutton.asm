...
Set_Pin:
		...
		bres	PB_DDR,#7 ;set PB2 as INPUT
...
End_250us:	;дд250 usдддддддддддддддддддддддд
		ld	A,TM_250US
		sub	A,#20			; for 250 x 20 = 5 ms counter
		jruge	SVR_5ms
		jp	MAIN_LP
;дддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддд
;		[  5 MSEC. SERVICE ROUTINE  дд 
;дддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддд
SVR_5ms:	ld	TM_250US,A
...
SVR_100ms:	ld	TM_005MS,A
		inc	TM_100MS
Start_100ms:	;дд100 msдддддддддддддддддддддддд
...
End_100ms:	;дд100 msдддддддддддддддддддддддд
		ld	A,TM_100MS
		sub	A,#10
		jruge	SVR_1SEC
		jp	MAIN_LP
;дддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддд
;		[  1 SEC. SERVICE ROUTINE  ] 
;дддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддд
SVR_1SEC:	ld	TM_100MS,A
	inc	TM_01SEC
Start_1sec:	;дд1 secддддддддддддддддддддддддд
	
End_1sec:	;дд1 secддддддддддддддддддддддддд
	jp	MAIN_LP
;мммммммммммммммммммммммммммммммммммммммммммммммммммммммммммммммммммммммммммммммммммммммм	
...
button_2:
	...
	jp chk_normal
normal_1:
	mov old_state1,state1
	mov state1,#0
	dec num
	jp display
normal_2:
	mov old_state2,state2
	mov state2,#0
	inc num
	jp display	
...
;WAIT_CLK:	;ld	A,CLK_CMSR
		...
		;mov 	UART1_CR1,#$14		; 9 data(MSB is Parity bit), UART enable,even Parity
		;mov 	UART1_CR3,#$00		; 1 stopbit, Disable SLK
		;mov 	UART1_BRR2,#$01
		;mov 	UART1_BRR1,#$34		; 19200 bps / Fmaster = 16 MHz
		;mov 	UART1_CR2,#$24		; OFF TIEN, ON RIEN, OFF TEN, ON REN 		
		;ld	A,UART1_SR
		;ld	UART1_DR,A		; Clr TXE bit
		;дд ENABLE CLOCK дддддддддддддддд	
		mov	CLK_PCKENR1,#%10010000	;ENABLE CLOCK FOR TIMER 1,4	
		mov	CLK_PCKENR2,#%00000000		
...
INIT_PORT:	;дд INIT PORT A ддддддддддддддддд
		...
		mov	PD_ODR,#%00000000    	;© PD3-0 Output push-pull                                                              
		mov	PD_DDR,#%00011111  	;Ё PD4   Output IR Transmit                                                                  
		mov	PD_CR1,#%00011111   	;Ё PD6,5 UART / PD7 Input with Interrupt for IR                                                                   
...
;дддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддд
	...
	DC.L {$82000000+NONHANDLEDINTERRUPT}	; IRQ17   UART1 TX COMPLETE     				0X00 804C                            
	DC.L {$82000000+NONHANDLEDINTERRUPT}	; IRQ18   UART1 RECEIVE REGISTER DATA FULL 			0X00 8050             
